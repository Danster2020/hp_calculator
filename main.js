...
function addCourse() {
    ...
    let c_term = document.getElementById("term").value;
    let c_sections = addSections();
    let c_total_points = document.getElementById("course_points").value;
...
function updateFinishTerm(element_id, course_id, term_index) {
    ...
    let new_value = document.getElementById(element_id).value;
...
function addSections() {
    ...
    for (let i = 0; i < section_query_name.length; i++) {
        sections.push(new Section(null, section_query_name[i].value, section_query_points[i].value, section_query_terms[i].value));
    }
